...
function loadUserSettings() {
    ...
    vibrationRequest.onsuccess = (event) => {
        if (vibrationRequest.result) {
            ...
            window.vibrationEnabled = vibrationEnabled; // تحديث متغير عام
    ...
    soundRequest.onsuccess = (event) => {
        if (soundRequest.result) {
            ...
            window.soundEnabled = soundEnabled; // تحديث متغير عام
            
            // تحديث نظام الصوت مباشرة
            if (audioSystem) {
    ...
    notificationsRequest.onsuccess = (event) => {
        if (notificationsRequest.result) {
            ...
            window.notificationsEnabled = notificationsEnabled; // تحديث متغير عام
...
function setupSettingsEventListeners() {
    // تبديل الصوت - تحديث فوري
    const soundToggle = document.getElementById('sound-toggle');
    if (soundToggle) {
        soundToggle.addEventListener('change', (e) => {
            soundEnabled = e.target.checked;
            window.soundEnabled = soundEnabled; // تحديث فوري
            ...
            if (soundEnabled && window.vibrationEnabled) {
                vibrate();
            }
            
            // تشغيل صوت التجربة
            if (soundEnabled) {
                // استخدام نظام الصوت مباشرة
                if (audioSystem && audioSystem.play) {
                    audioSystem.play('click');
                }
            }
            
            // حفظ الإعداد في الخلفية (غير متزامن)
            setTimeout(() => {
    ...
    // تبديل الاهتزاز - تحديث فوري
    const vibrationToggle = document.getElementById('vibration-toggle');
    if (vibrationToggle) {
        vibrationToggle.addEventListener('change', (e) => {
            vibrationEnabled = e.target.checked;
            window.vibrationEnabled = vibrationEnabled; // تحديث فوري
            
            // تفعيل الاهتزاز فوراً إذا تم التفعيل
    ...
    // تبديل الإشعارات - تحديث فوري
    const notificationsToggle = document.getElementById('notifications-toggle');
    if (notificationsToggle) {
        notificationsToggle.addEventListener('change', (e) => {
            notificationsEnabled = e.target.checked;
            window.notificationsEnabled = notificationsEnabled; // تحديث فوري
            
            // تفعيل/تعطيل الإشعارات فوراً
...
// حفظ الإعدادات في قاعدة البيانات (غير متزامن)
function saveSetting(settingId, value) {
    if (!db) return;
    
    // استخدام setTimeout للتأكد من عدم حجب الواجهة
    setTimeout(() => {
...
function handleColorChange(colorOption) {
    ...
    // تشغيل تأثير فوري
    if (window.soundEnabled && audioSystem) {
        audioSystem.play('click');
    }
    if (window.vibrationEnabled && 'vibrate' in navigator) {
