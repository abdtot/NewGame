...
// عرض لغز ثقافي بعد إكمال مستوى
function showCulturalPuzzle() {
    if (culturalPuzzles.length === 0) return false;
    
    // اختيار لغز عشوائي
    const randomIndex = Math.floor(Math.random() * culturalPuzzles.length);
    const puzzle = culturalPuzzles[randomIndex];
    
    // إنشاء عناصر واجهة المستخدم للغز
    const puzzleHTML = `
        <div class="cultural-puzzle-overlay" id="cultural-puzzle">
            <div class="cultural-puzzle-container">
                ...
                <button class="puzzle-close" id="puzzle-close">متابعة</button>
            </div>
        </div>
    `;
    ...
    document.querySelectorAll('.puzzle-option').forEach(option => {
        option.addEventListener('click', function() {
            ...
            if (selectedIndex === puzzle.correctAnswer) {
                ...
                // تحديث إنجاز سيد الألغاز
                updatePuzzleAchievement();
                
            } else {
                resultElement.innerHTML = '<i class="fas fa-times-circle"></i> إجابة خاطئة. حاول مرة أخرى في المرة القادمة.';
    ...
    document.getElementById('puzzle-close').addEventListener('click', function() {
        document.getElementById('cultural-puzzle').remove();
    });
...
// تحديث إنجاز سيد الألغاز
function updatePuzzleAchievement() {
    const transaction = db.transaction(['stats'], 'readwrite');
    const statsStore = transaction.objectStore('stats');
    
    // الحصول على عدد الألغاز الصحيحة الحالي
    const request = statsStore.get('puzzlesSolved');
    
    request.onsuccess = function(event) {
        let puzzlesSolved = event.target.result ? event.target.result.value : 0;
        puzzlesSolved++;
...
function startDailyChallenge() {
    ...
    showScreen('game-screen');
    document.getElementById('current-stage').textContent = 'تحدي';
    document.getElementById('current-level').textContent = 'يومي';
    document.getElementById('level-points').textContent = '0';
    updateProgress('level-progress', 0, 3);
    ...
    createChallengeCards(challenge, "daily");
}
...
function startTimedChallenge() {
    ...
    showScreen('game-screen');
    document.getElementById('current-stage').textContent = 'تحدي';
    document.getElementById('current-level').textContent = 'السرعة';
    document.getElementById('level-points').textContent = '0';
    updateProgress('level-progress', 0, 3);
    ...
    createChallengeCards(challenge, "timed");
}
...
function completeLevel() {
    ...
    transaction.oncomplete = () => {
        ...
        // عرض لغز ثقافي بعد إكمال المستوى (بعد تأخير بسيط)
        setTimeout(() => {
            const puzzleShown = showCulturalPuzzle();
            if (!puzzleShown) {
                // إذا لم يتم عرض لغز، انتقل مباشرة إلى شاشة المستويات
...
function showMessage(title, text, isLevelComplete = false) {
    ...
    messageBtn.onclick = () => {
        ...
        if (isLevelComplete) {
            // إذا اكتمل المستوى، لا ننتقل مباشرة إلى شاشة المستويات
            // لأننا قد نعرض لغزاً ثقافياً أولاً
            if (!document.getElementById('cultural-puzzle')) {
